# Implementation Plan: morphiq-ips-daemon

## Overview

Implement MorphIQ as an async Python daemon with a three-stage IPS pipeline (heuristic → anomaly → LangGraph agent), a local SQLite store, iptables/ufw firewall control, and a `morphiq` CLI. Each task builds incrementally toward a fully wired daemon process.

## Tasks

- [ ] 1. Project structure, dependencies, and core data models
  - Create package layout: `morphiq/`, `morphiq/cli/`, `morphiq/pipeline/`, `morphiq/store/`, `morphiq/fw/`, tests directory
  - Define `LogEntry`, `PatternRule`, `FilterResult`, `FeatureVector`, `BanRecord`, `AuditEvent`, `TrafficRecord`, `LLMAssessment`, `AgentDecision`, `Action` dataclasses and enums in `morphiq/models.py`
...
- [ ] 2. Config loading and validation
  - [ ] 2.1 Implement `Config` dataclass and `ConfigLoader` in `morphiq/config.py`
...
- [ ] 3. SQLite store
  - [ ] 3.1 Implement `SQLiteStore` in `morphiq/store/sqlite_store.py`
...
- [ ] 4. Log tailing and parsing
  - [ ] 4.1 Implement `LogTailer` in `morphiq/pipeline/log_tailer.py`
...
- [ ] 5. Heuristic filter
  - [ ] 5.1 Implement `HeuristicFilter` in `morphiq/pipeline/heuristic_filter.py`
...
- [ ] 6. Anomaly detector
  - [ ] 6.1 Implement `AnomalyDetector` in `morphiq/pipeline/anomaly_detector.py`
...
- [ ] 8. LLM client
  - [ ] 8.1 Implement `LLMClient` in `morphiq/llm_client.py`
...
- [ ] 9. LangGraph Agent
  - [ ] 9.1 Implement `Agent` in `morphiq/pipeline/agent.py` with LangGraph state machine
...
- [ ] 10. Firewall controller
  - [ ] 10.1 Implement `FirewallController` in `morphiq/fw/firewall_controller.py`
...
- [ ] 11. Cooldown scheduler
  - [ ] 11.1 Implement `CooldownScheduler` in `morphiq/fw/cooldown_scheduler.py`
...
- [ ] 13. CLI
  - [ ] 13.1 Implement `morphiq` CLI in `morphiq/cli/main.py` using `typer`
    - `morphiq status`: daemon state (PID file + process liveness), active ban count, pipeline stage counters
    - `morphiq audit [--limit N]`: return `min(N, M)` most recent `threat_audit_log` records ordered by `occurred_at` DESC
    - `morphiq ban list`: display all `active_bans` records (IP, reason, expiry)
    - `morphiq unban <ip>`: call `FirewallController.unblock()` + delete `active_bans` record; informative message + exit 0 if IP not banned
    - `morphiq whitelist check <ip>`: call `FirewallController.is_whitelisted()`
  ...
  - [ ]* 13.2 Write property test for audit order and count (Property 17)
    - **Property 17: morphiq audit --limit N returns correct count and order**
  ...
  - [ ]* 13.4 Write unit tests for CLI
    - Test `morphiq unban` for non-banned IP; test offline daemon message; test `morphiq status` output format
    - _Requirements: 10.5, 10.7_

- [ ] 14. Config hot-reload watcher
  - Implement `ConfigWatcher` using `watchdog` (inotify) in `morphiq/config.py`
...
- [ ] 15. Daemon supervisor and lifecycle management
  - [ ] 15.1 Implement `Daemon` class in `morphiq/daemon.py`
...
- [ ] 16. systemd unit file
  - Create `morphiq.service` unit file template
