# Design Document: morphiq-ips-daemon

## Overview

MorphIQ is a Python daemon implementing a three-stage Intrusion Prevention System (IPS) pipeline for Linux servers. It tails web server access logs in real time, passes entries through a heuristic filter, then an ML-based anomaly detector, and finally a LangGraph-orchestrated LLM agent that decides whether to block an IP via iptables/ufw. All state lives in a local SQLite database; a `morphiq` CLI exposes status, audit, and ban-management commands. The system is designed for zero-configuration privacy-preserving operation: no data ever leaves the host.
...
Responsibilities: provide `morphiq` command-line interface.
...
```
morphiq status                  # daemon state, active ban count, pipeline counters
morphiq audit [--limit N]       # N most recent threat_audit_log records
morphiq ban list                # all active_bans records
morphiq unban <ip>              # remove DROP rule + active_bans record
morphiq whitelist check <ip>    # whether IP is whitelisted
```

- All commands check daemon liveness (PID file + process check).
- If daemon offline: `morphiq status` shows "offline"; all other commands display message and exit non-zero.
- `morphiq unban` for non-banned IP: informative message, exit 0.
...
llm_model_path: /opt/morphiq/models/gemma-2b.gguf
default_ban_duration_s: 3600       # 1 hour
traffic_retention_s: 86400         # 24 hours
poll_interval_s: 60                # Cooldown_Scheduler interval
pid_file_path: /var/run/morphiq.pid
db_path: /var/lib/morphiq/morphiq.db
heuristic_patterns:
  - name: sql_injection
    field: path
...
*For any* integer `N >= 0` and any `threat_audit_log` with `M` records, `morphiq audit --limit N` returns exactly `min(N, M)` records, ordered by `occurred_at` descending (most recent first).
...
*For any* IP address string, the output of `morphiq whitelist check <ip>` matches the boolean result of `FirewallController.is_whitelisted(ip)`. There is no IP for which the CLI and the in-process function disagree.
...
- `CLI`: `morphiq unban` for non-banned IP; offline daemon message; `morphiq status` format
...
| P17: morphiq audit order/count | Random N values and random audit log sizes |
...
```python
# Feature: morphiq-ips-daemon, Property 1: Log parse round-trip
```
...
- `morphiq` CLI commands end-to-end against a populated SQLite test database
