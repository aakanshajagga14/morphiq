...
from snoop.config import load_config
from snoop.store.sqlite_store import SQLiteStore
from snoop.fw.firewall_controller import FirewallController
from snoop.models import FeedbackEvent, Action
from snoop.pipeline.anomaly_detector import AnomalyDetector
from snoop.cli.setup_wizard import setup_interactive

app = typer.Typer(
    name="snoop",
    help="[bold blue]Snoop IPS[/bold blue] — AI-powered intrusion prevention for Windows",
    rich_markup_mode="rich",
    add_completion=False,
)
console = Console()

CONFIG_PATH: str = os.environ.get("SNOOP_CONFIG", "config.yaml")
...
def start(config: Optional[str] = typer.Option(None, "--config")):
    ...
    if python_exe.lower().endswith("snoop.exe"):
    ...
    if os.name == 'nt':
        ...
        subprocess.Popen(
            [launcher, "-m", "snoop.daemon", "--config", abs_cfg],
            cwd=cwd,
        ...
        subprocess.Popen(
            [python_exe, "-m", "snoop.daemon", "--config", abs_cfg],
    ...
    for _ in range(20):
        ...
        if pid_file.exists():
            ...
            console.print(f"[green]Snoop IPS started successfully (PID: {pid}).[/green]")
            return
    console.print("[red]Daemon did not start in time. Check snoop.log for details.[/red]")
    raise typer.Exit(1)
...
def stop(config: Optional[str] = typer.Option(None, "--config")):
    ...
    if not pid_file.exists():
        console.print("[yellow]PID file not found. Is Snoop running?[/yellow]")
        raise typer.Exit(1)
    ...
    console.print("[green]Snoop IPS stopped.[/green]")
