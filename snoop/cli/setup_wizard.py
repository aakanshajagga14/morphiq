...
╚══════════════════════════════════╝"""
    ...
    config_dict["db_path"] = "snoop.db"
    config_dict["pid_file"] = "snoop.pid"
    config_dict["snoop_log_file"] = "logs/snoop.log"
    ...
    if os.name == 'nt' and Confirm.ask("Do you want Snoop to start automatically on login?"):
        python_exe = sys.executable
        cmd = f'schtasks /Create /TN "Snoop IPS" /TR "\\"{python_exe}\\" -m snoop.daemon --config {config_path}" /SC ONLOGON /RL HIGHEST /F'
    ...
    console.print("1. Run: [cyan]snoop start[/cyan] (as Administrator)")
    console.print("2. View status: [cyan]snoop status[/cyan]")
    if dash_enabled:
        console.print("3. View dashboard: [cyan]snoop dashboard[/cyan]")
