...
from snoop.models import BanRecord
...
class FirewallController:
    ...
    def _block_cmd(self, ip: str) -> Optional[list[str]]:
        if self.config.fw_backend == "windows":
            return ["netsh", "advfirewall", "firewall", "add", "rule", f"name=Snoop-Block-{ip}", "dir=in", "action=block", f"remoteip={ip}", "protocol=any"]
    ...
    def _unblock_cmd(self, ip: str) -> Optional[list[str]]:
        if self.config.fw_backend == "windows":
            return ["netsh", "advfirewall", "firewall", "delete", "rule", f"name=Snoop-Block-{ip}"]
        elif self.config.fw_backend == "iptables":
