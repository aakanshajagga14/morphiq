...
from snoop.models import PatternRule
...
class Config:
    """
    Complete runtime configuration for the Snoop IPS daemon.
...
class ConfigLoader:
    ...
    The environment variable SNOOP_CONFIG may override the path given to
    ...
    def load(cls, path: str) -> Config:
        ...
        If the SNOOP_CONFIG environment variable is set it takes precedence
        over the *path* argument.
        """
        resolved = os.environ.get("SNOOP_CONFIG", path)
    ...
    def validate(cls, raw: dict[str, object]) -> Config:
        ...
        if not pid_raw.strip():
            if sys.platform == "win32":
                ...
                pid_path = str(Path(base).expanduser() / "Snoop" / "snoop.pid")
            else:
                pid_path = str(Path("/tmp/snoop.pid"))
        else:
            pid_path = pid_raw
        ...
        return Config(
            ...
            snoop_log_file=validated_strings["snoop_log_file"],
    ...
    *callback* (after a debounce period) whenever the file changes.
    ...
    Usage::
        ...
        watcher = ConfigWatcher(
            config_path="/etc/snoop/config.yaml",
...
def load_config(path: Optional[str] = None) -> Config:
    ...
    Resolution order for the config file path:
        1. SNOOP_CONFIG environment variable
        2. *path* argument
    ...
    """
    resolved = os.environ.get("SNOOP_CONFIG") or path or "config.yaml"
